...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::mac_cmds::get_accessibility_status,
            commands::mac_cmds::get_permissions_status,
            commands::mac_cmds::check_permissions,
            commands::mac_cmds::open_permission_settings_cmd,
            commands::mac_cmds::open_accessibility_settings_cmd,
            commands::mac_cmds::request_accessibility,
            commands::mac_cmds::request_accessibility_permission,
            commands::mac_cmds::request_microphone_permission,
            commands::mac_cmds::reveal_app_in_finder,
        ])
        .setup(|app| {
            ...
            // Log how macOS will identify us in Accessibility (bare binary vs .app).
            let ax = injection::accessibility_status_detail();
            eprintln!(
                "oto accessibility: trusted={} bundled={} name={} path={}",
                ax.trusted, ax.bundled, ax.display_name, ax.executable_path
            );
            if !ax.trusted {
                eprintln!(
                    "oto accessibility: not granted — enable “{}” under System Settings → Privacy & Security → Accessibility (use + if missing)",
                    ax.display_name
                );
            }
