...
<section class="space-y-6">
  <header>
    ...
    <p class="mt-1 text-sm text-slate-400">
      Oto is system-wide AI voice dictation for macOS.
    </p>
  </header>
  ...
  >
    <div class="flex items-baseline justify-between gap-4">
      ...
      <div class="text-right text-xs text-slate-500">dev.oto.mac</div>
    </div>
    ...
    <div class="space-y-2 border-t border-white/10 pt-4">
      ...
      <p class="text-sm leading-relaxed text-slate-400">
        ...
        the macOS Keychain. History is local, and sync only runs against the endpoint you choose.
    ...
    <div class="space-y-2 border-t border-white/10 pt-4 text-xs leading-relaxed text-slate-500">
      <p>
        ...
        <code class="rounded bg-white/5 px-1">~/Library/Application Support/dev.Oto.oto/</code>
        (or the equivalent project config directory).
      </p>
      <p>
        macOS port of the Linux Oto app — same pipeline, macOS Accessibility and Core Graphics
        for text insertion.
      </p>
    </div>
  </div>
